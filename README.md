...
**Left (QQ plot):** Under correct specification, standardised innovations should follow a standard normal distribution, placing all points on the reference line. The distribution is approximately normal with mild excess kurtosis (4.7 versus 3.0 for a Gaussian). This is primarily the known leptokurtosis of cryptocurrency returns. It may also partly reflect the QML approximation in the dual-observation design (see [Dual-observation design](#dual-observation-design)): treating `eta_t` as Gaussian when the true `log(chi^2(1))` noise is left-skewed can bias the Kalman gain's weighting of `h_t` updates during large moves, indirectly affecting the return equation's own residuals. This second effect has not been isolated or quantified here. Neither indicates model misspecification.
